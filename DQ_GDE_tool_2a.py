...
def main():
    ...
    if not st.session_state.check_results.empty:
        ...
        st.markdown("Do not run fixes more than once, or it might cause duplicates in the blocklist file!")
        ...
        fix_function_map = {
            ...
            "13_auto": "SELECT * FROM dq.fn_dq_gde_rb_m2m_fail('{csid}') WHERE excluded IS FALSE AND test_type_id IN (23,24) ORDER BY start_time",   #Updated!
    ...
    st.markdown("You can review the blocklist.text file in the folder where this script is saved")
